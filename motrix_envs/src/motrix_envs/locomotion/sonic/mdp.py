...
from motrix_envs.locomotion.wbt.mdp.command import _sample_motion_step
...
class SonicMotionCommandCfg(CommandCfg):
    ...
    def __call__(self, env: ManagerEnv) -> CommandTerm:
        ...
        shape = env.sim_data["tracked_body_pos"].shape[1:]
        return SonicMotionCommand(
            ...
            command_buffer=np.empty((env.num_envs, 2 * len(self.joint_names)), np.float32),
...
class SonicActorObservation:
    ...
    def compute(ctx: ManagerContext, out: np.ndarray, state: SonicActorObservation) -> None:
        ...
        frame[root_dim:pos_end] = ctx.sim["robot_dof_pos"] - action.default_angles
...
class SonicActorObservationCfg(ObservationTermCfg):
    ...
    def __call__(self, env: ManagerEnv) -> ObsTerm:
        joint_count = env.sim_data["robot_dof_pos"].shape[-1]
...
class SonicCriticObservation:
    ...
    def compute(ctx: ManagerContext, out: np.ndarray, state: SonicCriticObservation) -> None:
        ...
        rotate_vector(inverse, clip.reference_body_pos_w[step] - root_pos, out[offset : offset + 3])
        ...
        for i in range(body_count):
            start = offset + 3 * i
            rotate_vector(inverse, ctx.sim["tracked_body_pos"][i] - root_pos, out[start : start + 3])
        ...
        frame[6 : 6 + joint_count] = ctx.sim["robot_dof_pos"] - action.default_angles
