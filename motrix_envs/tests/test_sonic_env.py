...
def test_sonic_smoke_environment_compiles_and_steps() -> None:
    store = Path(__file__).resolve().parents[2] / "data" / "sonic" / "lafan1-pack-smoke"
    cfg = registry.make_env_config("g1-sonic-smoke")
    cfg.commands.motion.packed_store = str(store)
    env = registry.resolve("g1-sonic-smoke", env_cfg=cfg).make(num_envs=2, seed=1)

    state = env.init_state()
    assert state.obs.policy.shape == (2, 243)
    assert state.obs.value.shape == (2, 286)

    next_state = env.step(np.zeros((2, 29), dtype=np.float32))
